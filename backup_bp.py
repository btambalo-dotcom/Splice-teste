...
# === Upload por URL (contorna limite 413) ===
@backup_bp.route("/fetch", methods=["POST"])
def fetch():
    url = request.form.get("url", "").strip()
    if not url:
        flash("Informe uma URL direta para o arquivo de backup.", "warning")
        return redirect(url_for("backup_bp.index"))
    try:
        import requests
        backup_dir = get_backup_dir()
        ...
        ts = time.strftime("%Y%m%d-%H%M%S")
        # tenta inferir nome
        fname = url.split("/")[-1] or "backup.zip"
        if "?" in fname:
            fname = fname.split("?")[0]
        if not any(fname.lower().endswith(ext) for ext in (".zip",".db",".sqlite",".sqlite3",".sql")):
            # força .zip como padrão
            fname = fname + ".zip"
        dest = os.path.join(backup_dir, f"{ts}__{fname}")
        # stream download
        with requests.get(url, stream=True, timeout=60) as r:
            ...
            with open(dest, "wb") as f:
                for chunk in r.iter_content(chunk_size=1024*1024*8):
                    if chunk:
                        f.write(chunk)
        flash(f"Arquivo baixado para: {os.path.basename(dest)}", "success")
    except Exception as e:
        current_app.logger.exception("Falha ao baixar por URL: %s", e)
        flash(f"Falha ao baixar por URL: {e}", "danger")
    return redirect(url_for("backup_bp.index"))
