...
import os, sqlite3, shutil, time, zipfile, json, tempfile
...
def restore_from_full_zip(zip_path, merge_files=True):
    ...
    # 1) obrigatoriamente cria safety antes de tocar no sistema
...
# === aplicar mudanças imediatamente (auto-restart robusto) ===
try:
    ext = getattr(current_app, "extensions", {}) or {}
    sa = ext.get("sqlalchemy") if isinstance(ext, dict) else None
    if sa and hasattr(sa, "db"):
    ...
    current_app.logger.info(f"SQLAlchemy dispose pós-restauração: {_e}")

# limpar cache se existir
try:
    cache = ext.get("cache") if isinstance(ext, dict) else None
    if cache and hasattr(cache, "clear"):
        cache.clear()
except Exception as _e:
    current_app.logger.info(f"Cache clear pós-restauração: {_e}")

import threading, time as _t, os as _os, signal, os
if os.environ.get("AUTO_RESTART_ON_RESTORE", "1") == "1":
    def _kick_restart():
        _t.sleep(1.0)
        try:
            pid = _os.getpid()
