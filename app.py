# -*- coding: utf-8 -*-
# Clean header to avoid hidden chars and ensure valid syntax
...
# Symlinks de compatibilidade caso algum trecho do código use ./uploads ou ./workmaps
try:
    proj_root = os.path.abspath(os.path.dirname(__file__))
    links = {'uploads': UPLOAD_FOLDER, 'workmaps': WORKMAP_FOLDER}
    for name, target in links.items():
        link = os.path.join(proj_root, name)
        if os.path.isdir(link) and not os.path.islink(link):
            # migra conteúdo e remove pasta
            import shutil
            for fname in os.listdir(link):
                src = os.path.join(link, fname)
...
# Symlinks de compatibilidade: se o código referir './uploads' ou './workmaps', redireciona ao disco
try:
    proj_root = os.path.abspath(os.path.dirname(__file__))
    for name, target in [('uploads', UPLOAD_FOLDER), ('workmaps', WORKMAP_FOLDER)]:
        link = os.path.join(proj_root, name)
        if os.path.isdir(link) and not os.path.islink(link):
